 ...
 * @author ngochieu
 */
public class GamePlay extends javax.swing.JInternalFrame {
	...
	this.setLayout(new GridLayout(col,row));
...
private void handleClickButton(Point point) {
		...
		if (getScore(getMatrixBoard(), true, false) >= winScore) {
			JOptionPane.showConfirmDialog(null, "Chuc mung ban da chien thang","Thong bao",JOptionPane.INFORMATION_MESSAGE);
                        UserDAO.updateScore(user.getUsername());
                        UserDAO.updateTimePlayed(user.getUsername());
		...
		if (getScore(getMatrixBoard(), false, true) >= winScore) {
			JOptionPane.showConfirmDialog(null, "Ban da thua","Thong bao",JOptionPane.INFORMATION_MESSAGE);
                        UserDAO.updateTimePlayed(user.getUsername());
                        account acc =new account(user);
                        acc.setVisible(true);
                        this.getDesktopPane().add(acc);
